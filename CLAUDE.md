...
- **Galaxy View**: ~100 stars with parallax background starfield (3 layers), volumetric nebulae (3 layers per cloud, billboarded), bloom tint pass preserving spectral colors, dark dust lanes (FBM noise, NormalBlending), pulsating variable stars (~4%), stellar remnants (black holes, neutron stars, white dwarfs), particle stream warp trails on visited connections, asteroid belt & comet activity indicators in star tooltips, **cluster halos** (soft additive glow billboards per star cluster)
- **System View**: Ray-marched atmospheric scattering on planets, planet shadow on rings, configurable star surface rotation, decorative moons orbiting planets, enhanced asteroid belts, single comet per system with dual tails; black hole gravitational lensing shader, neutron star rotating beam cones, white dwarf rendering; **nebula gas wisps** (5 FBM noise billboard planes, 3 color presets); **flyable ship** (orbital state machine: parking orbit → parametric Hohmann ellipse transfer → approach → docked orbit, 60-particle thruster trail, burn phases with 5× particle intensity, proximity thruster boost near planets), **event indicator sprites** (pulsing gold dots when Event Scanner upgrade active), **planet status rings** (3-arc shader rings showing scan/mine/explore completion per planet)
...
ho/js/visual-fx.js     — VFX experiment module: 10 toggleable effects with VFX_SETTINGS live tuning (uniforms)
ho/experiment.html     — Experiment page — loads game + visual-fx.js, VFX toggle panel with sliders + color pickers
...
- Cluster halos: PlaneGeometry + dual-Gaussian glow shader per star cluster (centroid + radius computed from star positions), billboarded toward camera, AdditiveBlending, configurable intensity/Gaussian widths/size multiplier
- System nebula gas: 5 billboarded PlaneGeometry planes in system view background, FBM noise density shader with configurable color/density/noise scale/animation speed/edge falloff, AdditiveBlending, 3 color presets (purple/blue-teal/warm brown)
- Stellar remnants: black holes (dedicated BLACK_HOLE_FRAG — single-pass fullscreen quad at renderOrder:10; acceleration-based lensing a=-1.5/r³·pos with exact photon sphere at r=1.5rs; analytic disk plane intersection; front-to-back alpha compositing; sin-based Keplerian ring texture with animated radial rings + azimuthal arcs; D³ Doppler beaming with β=1/√r; gl_FragDepth=0 at shadow pixels for planet occlusion; depth sphere at 3× starRadius renderOrder:0; planet orbits ×2.5 in BH systems; tuneable uniforms u_innerFade/u_bendStr/u_stepScale/u_gamma), neutron stars (STAR_FRAG with extreme params + rotating ConeGeometry beams), white dwarfs (STAR_FRAG with small radius + high temp)
...
- Ship: 8-model collection (CONFIG.ships catalog), GLB models loaded in parallel via GLTFLoader at startup, cached in shipModelCache Map; each model centered/normalized to unit size, materials converted to custom ShaderMaterial with two-light model (key 82% + fill 22% + ambient 18%) and S-curve contrast boost; handles textures, vertex colors, and solid colors; Fresnel rim-glow hull energy field sphere (cyan/teal, breathing pulse 1.4 Hz + shimmer 9 Hz, additive blending); selected model stored in state.selectedShip, swappable live via swapShipModel(); ship picker UI (fullscreen glassmorphism overlay, grid of cards with 3D previews rendered via offscreen WebGLRenderer, cached as data URLs); "Change Ship" button in ship status panel
...
- **v6.5**: HDR planet rendering — overhauled planet fragment shader for vivid, high-contrast visuals: 1.4× saturation boost on base textures, smoothstep S-curve contrast (deeper darks, brighter brights), removed explicit desaturation step that was washing out bright areas; lighting contrast widened (ambient 0.015→0.008, lit 0.65→0.85); ACES exposure raised from 1.2× to 1.6×, gamma changed from 0.96 (lifting) to 1.05 (deepening); specular highlights roughly doubled on water/ocean/ice, new specular on desert (mineral sheen) and gas giants (cloud sheen); atmosphere rim glow strengthened; per-type overlay improvements (stronger gas giant turbulence/wisps, richer desert dust, deeper ice cracks, brighter lava magma); warm/cool seed shift increased for more color variety between planets
- **v6.6**: GLB ship model — replaced procedural ship geometry (cones, cylinders, triangle wings, fins, nacelles, torus stripe) with external GLB model (`ho/models/spaceship.glb`, 472 vertices, 3 materials); loaded via GLTFLoader at module level with async cache (20KB, near-instant); model centered, normalized to unit size, wrapped in pivot Group; materials converted to MeshBasicMaterial (unlit, matching game aesthetic); `CONFIG.ship.meshScale` now directly controls ship size in game units (0.05, reduced from 0.5 to fix ship appearing system-sized); engine glow sprites repositioned relative to meshScale; all orbital state machine, thruster particles, and flight code unchanged
- **v6.7–6.8**: Ship scale tuning (meshScale 0.05 → 0.005 → 0.00125); multi-ship collection — 8 GLB models (Starfarer, Starfarer Mk II, Corsair, Interceptor, Vanguard, X-Wing, Hubble Probe, Sputnik) in `ho/models/`; CONFIG.ships catalog array; all models preloaded in parallel; selectedShip persisted in state with migration; ship picker overlay (glassmorphism, "Change Ship" button in ship panel); live model swap via swapShipModel()
- **v6.9**: Fixed ship rendering — material conversion now handles arrays, textures, vertex colors, transparency, alpha maps, and side; 3D ship previews in picker (offscreen WebGLRenderer with MeshStandardMaterial + ambient/directional lights, cached as data URLs)
- **v6.10**: Fixed ship scale bug — `setScalar` was overwriting unit normalization, making ship size depend on raw model geometry; changed to `multiplyScalar` so meshScale is now the true ship size in world units (0.8); all models render at consistent size regardless of source geometry dimensions
- **v6.13**: Ship size slider — range input (0.1–5.0) in ship picker overlay; persisted to `state.shipScale`; live preview via `swapShipModel()` on change; default hardcoded to 0.3 (ideal size)
- **v6.15**: Fullscreen button in both views — fullscreen toggle button visible in galaxy view and system view
- **v6.16**: Fullscreen idle auto-hide — all UI elements fade out after 3s of mouse inactivity in fullscreen mode; restored on any mouse movement; `body.ui-hidden` CSS class toggles opacity/pointer-events on all panels
- **v6.17**: Galaxy star color tuning — reduced saturation in GALAXY_STAR_FRAG (mix factor 1.6→0.85) without reducing brightness (halo multiplier and lumComp unchanged at 1.5/0.5)
- **v7.0**: Procedurally infinite multi-galaxy — one wormhole anomaly star placed per galaxy (far from home, deterministic via mulberry32(seed+12345), top 30% farthest eligible stars); wormhole rendered as animated cyan/violet spiral vortex (new aRemnantType=4.0 GLSL case); clicking wormhole star shows one-way warning card; on confirm: current galaxy archived to `state.galaxyChain`, per-galaxy state reset (visitedStars/reachableStars/planetActions), new galaxy generated from derived seed, fade transition with palette change; 5 cycling visual palettes (Teal Expanse → Azure Deep → Crimson Void → Emerald Reach → Amber Drift) governing bg star colors, nebula hues, and warp trail tints; `#hud-galaxy` label shows current galaxy name from gen 2 onward; journal `galaxy_transit` entries; old saves migrate cleanly (galaxyGeneration defaults to 0)
- **v7.1**: Wormhole icon marker — pulsing cyan/violet ring with 3-arm spinning swirl above wormhole star in galaxy view, rendered as Points geometry with WORMHOLE_MARKER_VERT/FRAG shaders; synchronized time uniforms with ship marker for consistent animation
- **v7.2**: Ship marker redesign — replaced flat diamond glow with pulsing outer ring and 4-arm spinning swirl (matching wormhole marker style), maintaining green color and additive blending in galaxy view
- **v7.3**: Hull energy field — Fresnel rim glow sphere around ship in system view with cyan/teal edge highlight, breathing pulse (1.4 Hz), and subtle shimmer (9 Hz); additive blending, rebuilt on ship swap to match current scale; parking orbit speed halved (0.18 → 0.09 rad/s) for slower star orbit
- **v7.4**: Ship material contrast — replaced MeshBasicMaterial (flat/unlit) with custom ShaderMaterial per mesh: two-light model (key light upper-front 82% + fill lower-back 22% + ambient 18%) with S-curve contrast boost (50% blend); handles texture maps, vertex colors, and solid color variants across all 8 ship models
- **v7.5**: Comet corona fade + system panel expansion — comets smoothly fade to invisible when entering star corona (r<6 → r<3.5, coronaFade multiplier on coma sprite and particle tails); ship picker preview fixed to handle custom ShaderMaterial by extracting texture/color from uniforms; system panel expanded (180→250px scrollable): planet list with 48×48 rendered thumbnails, per-type color badges, habitability scores, S/M/E completion indicators (green when done), anomalies section with divider
- **v7.6**: System panel planet thumbnails — circular crop (border-radius:50%, 56px diameter) matching info card styling
- **v7.7**: System panel snapshot caching — thumbnails now reuse the exact same dataURL captured in info cards (via cachePlanetSnapshot()) instead of eager re-capture, eliminating dark/incorrect renders from u_time=0 artifacts; cache cleared on system exit
- **v7.8**: Black hole planet occlusion — add invisible depth sphere sized to photon shadow radius (starRadius × 2.6), blocking planets behind BH; clamp spiral noise ≥0 in evalDisk to prevent negative density dark artifacts
- **v7.9**: Black hole depth via gl_FragDepth — shadow pixels (ray hits event horizon) write gl_FragDepth=0.0 so planets fail depth test; non-shadow pixels write gl_FragDepth=0.9999 so halo/disk don't occlude orbiting planets; enable BH ShaderMaterial depthWrite
- **v7.10**: Black hole depth writes fix — WebGL ES requires depthTest:true for gl_FragDepth writes to reach the buffer; v7.9 had depthTest:false so shadow occlusion never worked
- **v7.11**: Black hole shader rewrite — replace Velocity Verlet geodesic solver with clean acceleration-based lensing (a = -1.5/r³ · pos, exact photon sphere at r=1.5rs); analytic disk plane intersection replaces sign-change tracking; front-to-back alpha compositing; sin-based Keplerian ring texture (animated radial rings + azimuthal arcs) replaces simplex noise; Doppler D³ beaming with β=1/√r; gl_FragDepth=0 at shadow pixels; drops NOISE_GLSL dependency
- **v7.12**: Black hole shader refinement — port lab-proven shader with layered Keplerian ring texture, tunable D³ Doppler beaming; add uniforms u_innerFade, u_bendStr, u_stepScale, u_gamma for visual control
- **v7.13**: Two-pass black hole rendering — disk mesh at renderOrder:-1 + shadow mesh at renderOrder:10 to prevent shadow overdrawing disk; proportional step size cap (r×0.15 vs fixed 0.6) fixes BH disappearing at max zoom-out; planet orbitRadius ×2.5 in black hole systems
- **v7.14**: Single-pass black hole — collapse two-pass into one fullscreen quad at renderOrder:10: disk pixels draw disk color, captured pixels draw solid black, background draws lensed stars; depth sphere at renderOrder:0 occludes planets; eliminates black sphere overwriting bright ring artifact
...
- **v7.21**: Limit depth sphere to black holes only — normal stars don't need it (fullscreen quad covers the star area); the depth sphere was blocking transparent starfield Points behind it, creating a visible black circle artifact
