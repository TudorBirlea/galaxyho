...
- Ship: 8-model collection (CONFIG.ships catalog), GLB models loaded in parallel via GLTFLoader at startup, cached in shipModelCache Map; each model centered/normalized to unit size, materials converted to MeshBasicMaterial (preserving textures, vertex colors, transparency, material arrays); selected model stored in state.selectedShip, swappable live via swapShipModel(); ship picker UI (fullscreen glassmorphism overlay, grid of cards with 3D previews rendered via offscreen WebGLRenderer + MeshStandardMaterial + lighting, cached as data URLs); "Change Ship" button in ship status panel
