...
- **System View**: Ray-marched atmospheric scattering on planets, planet shadow on rings, configurable star surface rotation, decorative moons orbiting planets, enhanced asteroid belts, single comet per system with dual tails; black hole gravitational lensing shader, neutron star rotating beam cones, white dwarf rendering; **flyable ship** (orbital state machine: parking orbit → gravity slingshot / Hohmann transfer → approach → docked orbit, 60-particle thruster trail, burn phases with 5× particle intensity, slingshot thruster boost at closest approach), **event indicator sprites** (pulsing gold dots when Event Scanner upgrade active), **planet status rings** (3-arc shader rings showing scan/mine/explore completion per planet)
...
- Ship: THREE.Group (dual-cone fuselage + cockpit dome + delta wings + dorsal fin + engine nacelles + 3 glow sprites + hull accent stripe), scaled by CONFIG.ship.meshScale
  ...
  - Transfer: gravity slingshot (cubic Bezier curved around intermediate planet) when available, else Hohmann-like half-ellipse; duration from Kepler's 3rd law (scaled for gameplay, ×1.15 for slingshots), easeInOutCubic
  - Slingshot detection: `findSlingshotPlanet()` scans for intermediate planet between departure/target orbits (closest to midpoint radius, min 0.5 gap), predicts positions at midpoint/arrival, builds Bezier control points pulled toward slingshot planet (pull factor 0.6)
  - Slingshot visuals: 3× thruster boost near closest approach (|t-0.5| < 0.08), natural speed variation from Bezier curvature
